    ...
    <div className="w-[270px] h-[350px] bg-white rounded-lg cursor-pointer overflow-hidden group hover:shadow-xs transition-all duration-500 relative mt-10">
      ...
      <div className="p-3">
        ...
        <div className="flex items-center gap-2 mb-1">
          <span className="text-red-500 font-semibold text-[15px]">${item.price}</span>
          <span className="text-gray-400 line-through text-[13px]">{item.oldPrice}</span>
        </div>
        ...
        <div className="flex items-center gap-0.5">
          ...
          <span className="text-[12px] text-gray-500 ml-1">({item.rating})</span>
        </div>
      </div>
    </div>
  );
